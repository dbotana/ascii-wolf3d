...
short of anything solid or anyone alive**. If it cannot move at all it returns
silently rather than announcing itself — a wedged secret should not give away
that it is a secret. That silence is also why `validate-level.js` checks for
push-walls that can never move: the engine will not tell you.
...
The gap: `pushSecret` asks `cellAt` about walls and `occupied` about bodies, but
nothing about items. A slab that lands on a pickup makes that tile solid and the
pickup unreachable forever. No legal push in the shipped level does this, so it
is latent rather than live; see `todo.md`.
...
with `dying` as a brief animation state before `dead`. Line of sight is a
...
`LEVEL_1` is an array of strings, one character per tile. Entity characters are
...
The current floor is 40x40: 12 enemies, 22 pickups, 4 doors, 3 push-walls. The
intended route is red keycard → red door → blue keycard → blue door → elevator.
These numbers move as the level is edited — `run-tests.js` deliberately does not
assert on them, because that is `validate-level.js`'s job.
...
```sh
node reference/run-tests.js          # 74 assertions against the real game loop
...
push-walls, death, restart, level clear.

**Mutation-tested, and this is the part that matters.** Sixteen bugs have been
injected into copies of the game; the suite catches all sixteen. It did not
always: three push-wall mutations (a slab that blocks neither rays nor bodies,
and a secret pushable forever) passed a fully green suite, because nothing
asserted those behaviours. Green is not the same as covered. When you add a
system, add a mutation for it and confirm the suite goes red.
...
- A `# WOLFENSTAIN 3D — engine reference
...
- Four pickups shipped sealed inside the lobby kiosks with no opening. This is
  exactly what `validate-level.js` now catches, and the reason it exists.
 in an unquoted shell heredoc interpolated and corrupted `README.md`.
  Write repo files with the Write tool or a quoted heredoc.
- `pushSecret` guards its path with `cellAt` and `occupied` but not items, so a
  slab can land on a pickup and seal it away for good. Latent in the shipped
  level — no legal push there lands on one — and tracked in `todo.md`. Any new
  guard on a moving thing needs to ask what else can occupy a tile.
